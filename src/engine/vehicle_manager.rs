...
use crate::physics::{Vehicle, Helicopter, TrackedVehicle};
...
impl VehicleManager {
    ...
    /// Обновляет состояние лебёдки
    pub fn update_winch(&mut self, dt: f32) {
        // Логика обновления лебёдки
        self.winch.update(dt);
    }
...
mod tests {
    ...
    fn test_spawn_wheeled_vehicle() {
        ...
        assert!(result.is_ok());
        assert!(manager.has_active_vehicle());
        assert_eq!(manager.get_active_vehicle_type(), Some(VehicleType::Wheeled));
    }
