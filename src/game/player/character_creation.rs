...
use crate::game::player::{Player, Appearance};
...
impl CharacterCreation {
    ...
    pub fn prev_face(&mut self) {
        self.face_variant = if self.face_variant == 0 { 7 } else { self.face_variant - 1 };
        self.appearance.face_variant = self.face_variant;
    }
    ...
    pub fn prev_hair_style(&mut self) {
        self.hair_style = if self.hair_style == 0 { 9 } else { self.hair_style - 1 };
        self.appearance.hair_style = self.hair_style;
    }
