...
impl DebugMenu {
    ...
    pub fn render(&self, draw_text: &mut dyn FnMut(&str, f32, f32, [f32; 4])) {
        ...
        draw_text(&format!("Frame Time: {:.2} ms", self.frame_time_ms), x, y, white);
        y += line_height;
        ...
        draw_text(&format!("Active RigidBodies: {}", self.active_rigid_bodies), x, y, white);
        y += line_height;
        
        draw_text(&format!("Collision Pairs: {}", self.active_collisions), x, y, white);
        y += line_height;
        
        draw_text(&format!("Contacts Resolved: {}", self.physics_stats.contacts_resolved), x, y, white);
        y += line_height;
        
        draw_text(&format!("Physics Step: {} μs", self.physics_stats.step_time_us), x, y, white);
        y += line_height;
        
        draw_text(&format!("Sleeping Bodies: {}", self.physics_stats.sleeping_bodies), x, y, white);
        y += line_height;
        ...
        draw_text(&format!("Active Chunks: {}", self.active_chunks), x, y, white);
        y += line_height;
    }
    ...
    pub fn render_player_info(
        ...
        draw_text(&format!("Stamina: {:.1}/{:.1}", player.stamina, player.max_stamina), x, y, white);
        y += line_height;
        
        draw_text(&format!("Inventory: {:.1}/{:.1} kg", player.inventory_weight, player.max_inventory_weight), x, y, white);
        y += line_height;
