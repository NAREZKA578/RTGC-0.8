...
impl Scene for PauseScene {
    ...
    fn render(&mut self, renderer: &mut crate::graphics::renderer::Renderer) -> Result<(), Box<dyn std::error::Error>> {
        ...
        unsafe {
            ...
            renderer.draw_rect(panel_x, panel_y, panel_width, panel_height, [0.1, 0.1, 0.15, 0.95]);
            
            // Title
            renderer.draw_text("ПАУЗА", w/2.0 - 40.0, panel_y + 40.0, 1.5, [1.0, 1.0, 1.0, 1.0]);
            
            // Menu options
            renderer.draw_text("Нажмите ESC для продолжения", w/2.0 - 130.0, panel_y + 100.0, 0.9, [0.8, 0.8, 0.8, 1.0]);
            renderer.draw_text("M - Главное меню", w/2.0 - 80.0, panel_y + 130.0, 0.9, [0.7, 0.7, 0.7, 1.0]);
        }
