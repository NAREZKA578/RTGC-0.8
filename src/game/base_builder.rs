...
impl StructureType {
    ...
    pub fn required_skills(&self) -> Vec<(SkillType, u8)> {
        match self {
            ...
            StructureType::ContainerShelter => vec![(SkillType::Construction, 3), (SkillType::Welding, 2)],
            StructureType::PrefabHouse => vec![(SkillType::Construction, 5)],
            
            StructureType::StoragePlatform => vec![(SkillType::Construction, 2)],
            StructureType::Warehouse => vec![(SkillType::Construction, 4), (SkillType::Logistics, 3)],
            StructureType::ColdStorage => vec![(SkillType::Construction, 5), (SkillType::Electrics, 4)],
            StructureType::FuelTank => vec![(SkillType::Construction, 3)],
            
            StructureType::RepairStand => vec![(SkillType::Mechanics, 3)],
            StructureType::Workshop => vec![(SkillType::Mechanics, 5), (SkillType::Construction, 4)],
            StructureType::Garage => vec![(SkillType::Construction, 4)],
            StructureType::WeldingStation => vec![(SkillType::Welding, 4), (SkillType::Electrics, 3)],
            ...
            StructureType::Helipad => vec![(SkillType::Construction, 6), (SkillType::Navigation, 4)],
            StructureType::CraneFoundation => vec![(SkillType::Construction, 5), (SkillType::Crane, 3)],
            StructureType::DrillPad => vec![(SkillType::Drilling, 4), (SkillType::Geology, 3)],
            StructureType::LoggingPad => vec![(SkillType::Logging, 4), (SkillType::Construction, 3)],
        }
    }
    ...
    pub fn update(&mut self, delta_seconds: f32) {
        for structure in &mut self.structures {
            ...
            if structure.is_complete() {
                ...
                structure.take_damage(decay_rate * delta_seconds * structure.structure_type.max_integrity());
            }
        }
    }
...
impl BaseBuildingSystem {
    ...
    pub fn create_base(
        ...
        let player_base_count = self.bases.values()
    ...
    pub fn delete_base(&mut self, base_id: u64) -> Option<PlayerBase> {
        self.pending_constructions.retain(|(bid, _)| *bid != base_id);
    ...
    pub fn build_structure(
        ...
        let base = self.bases.get_mut(&base_id)
    ...
    pub fn update(&mut self, delta_seconds: f32, current_time: u64) {
        ...
        let completed: Vec<u64> = self.pending_constructions.iter()
            .filter(|(_, completion)| *completion <= current_time)
            .map(|(id, _)| *id)
            .collect();
        
        for structure_id in completed {
            self.pending_constructions.retain(|(id, _)| *id != structure_id);
...
mod tests {
    ...
    fn test_create_base() {
        ...
        let base_id = system.create_base(
            "Test Base".to_string(),
            BaseType::Outpost,
