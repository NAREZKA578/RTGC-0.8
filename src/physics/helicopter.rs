//! Helicopter Physics Module for RTGC-0.7
//! 
...
impl HelicopterConfig {
    ...
    pub fn custom(
        ...
        Self {
            ...
            engine_type: if engine_power > 500000.0 { EngineType::Turboshaft } else { EngineType::Piston },
            max_engine_power: engine_power,
            max_engine_torque: engine_power / 400.0,
            idle_rpm: if engine_power > 500000.0 { 6000.0 } else { 500.0 },
            max_rpm: if engine_power > 500000.0 { 6500.0 } else { 2500.0 },
...
impl MainRotor {
    ...
    pub fn calculate_thrust(&self, air_density: f32, inflow_velocity: Vector3<f32>) -> Vector3<f32> {
        ...
        for i in 0..num_elements {
            ...
            let resultant_vel = (tangential_velocity * tangential_velocity + axial_velocity * axial_velocity).sqrt();
            ...
            let azimuthal_factor = 1.0 + 0.3 * (self.cyclic_pitch_longitudinal.abs() + self.cyclic_pitch_lateral.abs());
    ...
    pub fn update_rotor_speed(&mut self, engine_torque: f32, rotor_torque: f32, moment_of_inertia: f32, dt: f32) {
        let omega = self.current_rpm * 2.0 * PI / 60.0;
...
impl TailRotor {
    ...
    pub fn calculate_thrust(&self, air_density: f32, main_rotor_torque: f32) -> Vector3<f32> {
        ...
        let thrust_magnitude = dynamic_pressure * disk_area * thrust_coefficient * self.blade_count as f32;
...
impl TurboshaftEngine {
    ...
    pub fn calculate_available_torque(&self, current_rpm: f32) -> f32 {
        ...
        let torque_curve = 1.0 - ((current_rpm - self.max_rpm * 0.7).abs() / (self.max_rpm * 0.3)).max(0.0);
    ...
    pub fn update(&mut self, dt: f32, load_torque: f32) {
        if !self.is_running {
            // Остывание двигателя
            self.engine_temperature += (self.ambient_temperature - self.engine_temperature) * 0.01 * dt;
            return;
        }
        ...
        let target_temp = self.ambient_temperature + 150.0 * load_factor + 50.0 * self.throttle_position;
        self.engine_temperature += (target_temp - self.engine_temperature) * 0.05 * dt;
        self.engine_temperature = self.engine_temperature.clamp(self.ambient_temperature, 900.0);
...
impl HelicopterControls {
    ...
    pub fn apply_input(&mut self, collective_input: f32, longitudinal_input: f32, 
                       lateral_input: f32, pedal_input: f32, throttle_input: f32,
                       dt: f32, smoothing_factor: f32) {
        // Плавное изменение управляющих сигналов
        self.collective = self.lerp_smooth(self.collective, collective_input.clamp(0.0, 1.0), smoothing_factor, dt);
        self.cyclic_longitudinal = self.lerp_smooth(self.cyclic_longitudinal, longitudinal_input.clamp(-1.0, 1.0), smoothing_factor, dt);
        self.cyclic_lateral = self.lerp_smooth(self.cyclic_lateral, lateral_input.clamp(-1.0, 1.0), smoothing_factor, dt);
        self.tail_rotor_pedals = self.lerp_smooth(self.tail_rotor_pedals, pedal_input.clamp(-1.0, 1.0), smoothing_factor, dt);
        self.throttle = self.lerp_smooth(self.throttle, throttle_input.clamp(0.0, 1.0), smoothing_factor, dt);
    }
...
impl Helicopter {
    ...
    pub fn update(&mut self, dt: f32) {
        ...
        self.engine.update(dt, self.main_rotor.calculate_torque(self.air_density, self.velocity));
    ...
    fn apply_controls(&mut self) {
        ...
        self.main_rotor.cyclic_pitch_longitudinal = self.controls.cyclic_longitudinal * max_cyclic_pitch;
    ...
    fn calculate_forces_and_torques(&self) -> (Vector3<f32>, Vector3<f32>) {
        ...
        let main_rotor_thrust = self.main_rotor.calculate_thrust(self.air_density, body_velocity);
        ...
        let main_rotor_torque = self.main_rotor.calculate_torque(self.air_density, body_velocity);
        total_torque += Vector3::new(0.0, -main_rotor_torque, 0.0); // Против направления вращения
        
        // 3. Тяга хвостового ротора
        let tail_thrust = self.tail_rotor.calculate_thrust(self.air_density, main_rotor_torque);
    ...
    fn calculate_fuselage_drag(&self, relative_velocity: Vector3<f32>) -> Vector3<f32> {
        ...
        let drag_x = 0.5 * self.air_density * speed_squared * self.fuselage_drag_coefficient.x * self.side_area;
        let drag_y = 0.5 * self.air_density * speed_squared * self.fuselage_drag_coefficient.y * self.top_area;
        let drag_z = 0.5 * self.air_density * speed_squared * self.fuselage_drag_coefficient.z * self.fuselage_reference_area;
    ...
    fn calculate_ground_contact(&self) -> (Vector3<f32>, Vector3<f32>) {
        ...
        let normal_force = spring_constant * compression - damping_constant * self.velocity.dot(&self.ground_contact_normal);
