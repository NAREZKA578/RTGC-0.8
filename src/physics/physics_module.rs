...
pub use super::deformable_terrain::{DeformableTerrainComponent, DeformableTerrainInterface, DeformationType};
...
impl PhysicsWorld {
    ...
    pub fn step(&mut self, delta_time: f32) {
        ...
        for i in 0..self.sub_steps {
            ...
            if let Some(water_y) = self.water_plane_y {
                ...
                let buoyancy_data: Vec<(usize, Option<Vector3<f32>>)> = self.rigid_bodies
                    .iter()
                    .enumerate()
                    ...
                    .map(|(idx, body)| {
                        ...
                        if body_bottom < water_y && body_top > water_y {
                            ...
                            let buoyancy_force = Vector3::new(
                                0.0,
                                water_density * displaced_volume * 9.81,
                                0.0,
                            );
    ...
    fn handle_collisions_parallel(&mut self) {
        ...
        for (i, j) in &self.broadphase_pairs {
            if let Some(contact) = self.detect_collision(*i, *j) {
                ...
                let body_a_vel = self.rigid_bodies.get_by_index(*i).map(|b| b.velocity.magnitude()).unwrap_or(0.0);
                let body_b_vel = self.rigid_bodies.get_by_index(*j).map(|b| b.velocity.magnitude()).unwrap_or(0.0);
    ...
    fn detect_box_box_collision(&self, _i: usize, _j: usize, _body_a: &RigidBody, _body_b: &RigidBody, _extents_a: &Vector3<f32>, _extents_b: &Vector3<f32>) -> Option<Contact> {
