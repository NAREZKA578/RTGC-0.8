...
use crate::graphics::rhi::{
    ...
    PipelineStateObject, InputLayout, VertexAttribute, VertexFormat,
    PrimitiveTopology, ColorBlendState, DepthState, RasterizerState, CullMode, FrontFace,
    RenderPassDescription, RenderAttachment, LoadOp, StoreOp, ClearValue,
    Viewport, ScissorRect, ResourceState, ResourceBarrier,
    RhiResult,
};
...
impl RendererRhi {
    pub fn new(device: Arc<dyn IDevice>, width: u32, height: u32) -> Result<Self, Box<dyn std::error::Error>> {
    ...
    fn create_bitmap_font(device: &Arc<dyn IDevice>) -> Result<(ResourceHandle, HashMap<char, [f32; 4]>), Box<dyn std::error::Error>> {
        use std::collections::HashMap;
    ...
    pub fn prev_city(&mut self) {
        self.current_city_index = if self.current_city_index == 0 { 9 } else { self.current_city_index - 1 };
    }
}
